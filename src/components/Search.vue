...
export default {
  ...
  methods: {
    ...
    onSubmit() {
      this.$http.get(`?apikey=${api.key}&t=${this.search.searchTitle}${this.search.searchType ? `&type=${this.search.searchType}` : ''}${this.search.searchYear ? `&y=${this.search.searchYear}` : ''}`)
        .then(response => {
        ...
        .then(data => {
          if(data.Error) {
            this.showError = true;
            const errorString = `Failed search for "${this.search.searchTitle}"${this.search.searchYear ? ` + "${this.search.searchYear}"` : ''}${this.search.searchType ? ` + "${this.search.searchType}"` : ''}`;
